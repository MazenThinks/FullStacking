...
app.get("/filter", (req, res) => {
  ...
  const filteredJoke = jokes.filter((j) => j.jokeType === type);
...
app.put("/jokes/:id", (req, res) => {
  ...
  const index = jokes.findIndex((j) => j.id === i);
...
app.patch("/jokes/:id", (req, res) => {
  const i = parseInt(req.params.id);
  const existingJoke = jokes.find((j) => j.id === i);
  const updatedJoke = {
    id: i,
...
app.delete("/jokes/:id", (req, res) => {
  ...
  if (index > -1) {
    ...
    res
      .status(404)
      .json({ error: `Joke with id: ${i} not found. No jokes were deleted.` });
  }
});
